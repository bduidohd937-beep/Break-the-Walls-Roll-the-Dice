...
import { STORAGE_KEYS, loadNumber, saveJson, saveNumber } from "../storage";
...
export function useGatheringProduction(a:Args){
 useEffect(()=>{
  ...
  if(ticks>0&&(a.workers.wood||a.workers.stone)){
   ...
   if(wood||stone){a.setResources(current=>{const next={wood:current.wood+wood,stone:current.stone+stone};saveJson(STORAGE_KEYS.resources,next);return next});a.setOfflineGather({wood,stone,seconds})}
  }
  a.lastSeenRef.current=now;saveNumber(STORAGE_KEYS.gatherLastSeen,now);
  const timer=window.setInterval(()=>{const tick=Date.now();a.lastSeenRef.current=tick;saveNumber(STORAGE_KEYS.gatherLastSeen,tick);a.setResources(current=>{const next={...current};if(a.workers.wood)next.wood+=a.getAutoAmount("wood");if(a.workers.stone)next.stone+=a.getAutoAmount("stone");saveJson(STORAGE_KEYS.resources,next);return next})},3000);
